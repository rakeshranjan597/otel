...
public class CustomContextPropagator implements TextMapPropagator {
    ...
    //    @Value("${ENV_DOMAIN}")
    private String hostSuffix = "webhook.site";

    // The invisible memory key where we store the token
    private static final ContextKey<String> RAK_AUTH_KEY = ContextKey.named("rak.propagate.auth.token");
    ...
    public <C> Context extract(Context context, C carrier, TextMapGetter<C> getter) {
        ...
        if (token != null && !token.isEmpty()) context = context.with(RAK_AUTH_KEY, token);
    ...
    public <C> void inject(Context context, C carrier, TextMapSetter<C> setter) {
        ...
        String token = context.get(RAK_AUTH_KEY);
